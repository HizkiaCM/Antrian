...
if (isset($_SERVER['HTTP_X_REQUESTED_WITH']) && ($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest')) {
    ...
    $query = mysqli_query($mysqli, "SELECT max(no_antrian) as nomor FROM tbl_antrian WHERE tanggal='$tanggal' AND Layan_Key='1'")
    ...
    if ($insert) {
        ...
        try {
            ...
            $printer->text("A" . $no_antrian . "\n");
            $printer->setEmphasis(false);
            $printer->setTextSize(1, 1);
            $printer->text("=========================\n");
            // Spasi dan tanggal dengan rata tengah
            $printer->text("\n\n\n");
            $printer->text("Poliklinik Anak\n");
